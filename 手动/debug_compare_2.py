...
# 加载解密后的模型
decrypted_model_path = 'C:/Users/TCTrolong/Desktop/compare/777decrypted_model.pth'
    ...
    for key in keys1.intersection(keys2):
        ...
        if isinstance(param1, torch.Tensor) and isinstance(param2, torch.Tensor):
            ...
            if not torch.equal(param1, param2):
                print(f"参数 {key} 不一致.")
                print(f"原始模型参数均值: {param1.mean()}, 解密模型参数均值: {param2.mean()}, 平均差异: {(param1 - param2).abs().mean()}")
        else:
            # 打印非 Tensor 类型的参数信息
